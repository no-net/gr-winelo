...
This is achieved by replacing the RF hardware and the radio channel by virtual equivalents, which permits the simulation of GNU Radio flowgraphs under similar conditions to those would occur in real world tests.
...
For this example we decided to use the COST 207 typical urban model.
The server has to know the sample rate with which to flowgraphs are running to adjust the delays of the channel model to this rate.
*fmax* sets the maximum Doppler frequency for Doppler spectra in the channel model::
        ./server.py --model cost207typicalurban --opts sample_rate 10e6 fmax 100
       
Now that the server is running we can start to connect some GNU Radio flowgraphs.
Below you will find some example flowgraphs.
Notice that they are almost identical to usual flowgraphs instead that they are using WiNeLo sink/source blocks instead of the traditional UHD-equivalents.
...
* **The server does not transmit when I connect a flowgraph with more than one sink/source.** This problem is caused by the way the server handles new connections. One work-around is to connect a dummy flowgraph with only one WiNeLo sink or source block to the server to kickstart the server. After that the dummy flowgraph can be removed again.
* **The server crashes when I set a large packet size.** Somehow twisted can't cope with large packets. The maximum for me are 4000. So far the only solution is using smaller packets.
