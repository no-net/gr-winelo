...
class dopplerspecs():
    ...
    def __init__(self, N, fmax):
        ...
        self.A_2 = 10 ** 1.5 / (np.sqrt(2 * np.pi) * (np.sqrt(10) + 5)
                                * self.fmax)
    ...
    def _gen_jakes(self):
        sigma = 1
        psd = np.zeros(self.N)
        comp_jakes = lambda f: sigma ** 2 / \
            (np.pi * self.fmax * np.sqrt(1 - (f / self.fmax) ** 2))
    ...
    def _gen_gauss1(self):
        ...
        psd = self._gauss(A_1, f_1, s_1, self.freqs) + \
            self._gauss(A_2, f_2, s_2, self.freqs)
        return psd
